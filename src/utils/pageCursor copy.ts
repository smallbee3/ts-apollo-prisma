...
const PageCursor = new GraphQLObjectType<any, any>({
...
export const PageCursorsType = new GraphQLObjectType<any, any>({
...
export function createPageCursors(
  ...
  if (max % 2 === 0) {
    console.log(`Max of ${max} passed to page cursors, using ${max + 1}`);
    max = max + 1;
  }

  const totalPages = computeTotalPages(totalRecords, size);

  console.log(6, { totalPages });
  ...
  console.log(77, pageCursors, stringify(pageCursors));
